module "s3bucket_splunk_logs" {
  ...
  policy_documents = [
    data.aws_iam_policy_document.s3bucket_splunk_logs.json
  ]

  bucket_logging_target = {
    bucket = local.acct.s3_buckets["s3bucket_access_logs"]["id"]
  }
  ...
  default_tags = {
    Name = "Splunk Logs Bucket"
  }
}

data "aws_iam_policy_document" "s3bucket_splunk_logs" {
  statement {
