resource "aws_kinesis_firehose_delivery_stream" "splunk_firehose" {
  ...
  splunk_configuration {
    hec_endpoint            = data.aws_ssm_parameter.splunk_hec_endpoint.value
    hec_endpoint_type       = "Event"
    hec_token               = data.aws_ssm_parameter.splunk_hec_token.value
    retry_duration          = 300
    s3_backup_mode          = "FailedEventsOnly"
    s3_configuration {
      role_arn           = var.firehose_to_splunk_role_arn
